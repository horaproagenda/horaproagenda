...
const getAvailableSessionCount = (pkg: Pick<ClientPackage, 'total_sessions' | 'sessions_scheduled' | 'appointments'>) => {
...
export function useClientPackages(clientId: string | null) {
  ...
  const availablePackages = clientPackages.filter(pkg => getAvailableSessionCount(pkg) > 0);
  ...
  const getPackageRemainingSessions = (packageId: string) => {
    ...
    const remaining = getAvailableSessionCount(pkg);
    return {
