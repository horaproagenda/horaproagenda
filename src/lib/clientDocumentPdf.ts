...
/**
 * Generate a single combined PDF containing multiple filled documents.
 * Each document starts on a new page with its own title and header.
 */
export function generateCombinedClientDocumentsPdf(opts: {
  documents: Array<{ title: string; filledContent: string }>;
  header?: PdfHeaderInfo;
  fileName?: string;
}) {
  const { documents, header } = opts;
  ...
  documents.forEach((d, idx) => {
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text(removeAccents(d.title), pageWidth / 2, y, { align: 'center' });
    y += 12;
    ...
    if (header?.name) { doc.text(removeAccents(`Cliente: ${header.name}`), margin, y); y += 5; }
    if (header?.cpf) { doc.text(removeAccents(`CPF: ${header.cpf}`), margin, y); y += 5; }
    if (header?.birthdate) { doc.text(removeAccents(`Nascimento: ${header.birthdate}`), margin, y); y += 5; }
    if (header?.professionalName) { doc.text(removeAccents(`Profissional: ${header.professionalName}`), margin, y); y += 5; }
    y += 4;
    ...
    doc.setFontSize(11);
    const lines = doc.splitTextToSize(removeAccents(d.filledContent), maxWidth);
    for (const line of lines) {
      if (y > pageHeight - 35) {
        doc.addPage();
        y = 20;
  ...
  const fileName = opts.fileName || `Documentos - ${header?.name || 'Cliente'}.pdf`;
  doc.save(removeAccents(fileName));
}
