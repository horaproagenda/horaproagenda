...
BEGIN
  ...
  -- Desvincular registros financeiros/auditoria das vendas vinculadas
  UPDATE public.cash_transactions
  SET reference_id = NULL
  WHERE reference_type = 'package_refund'
    AND reference_id = ANY(v_sale_ids);

  UPDATE public.client_credit_transactions
  SET sale_id = NULL
  WHERE sale_id = ANY(v_sale_ids);

  UPDATE public.payments_audit
  SET single_sale_id = NULL
  WHERE single_sale_id = ANY(v_sale_ids);

  UPDATE public.cash_register_entries
  SET single_sale_id = NULL
  WHERE single_sale_id = ANY(v_sale_ids);
  ...
  -- Apagar os agendamentos do pacote para que sumam do histórico do cliente
  IF array_length(v_appointment_ids, 1) > 0 THEN
    -- Desvincular financial_entries (FK sem ON DELETE) para liberar a exclusão
    UPDATE public.financial_entries
    SET appointment_id = NULL
    WHERE appointment_id = ANY(v_appointment_ids);

    -- package_appointments.appointment_id, client_services, treatment_photos, product_daily_consumption,
    -- whatsapp_send_queue: ON DELETE SET NULL
    -- appointment_product_consumption, appointment_additional_items: ON DELETE CASCADE
    -- appointment_edit_locks / reminder_log / package_appointment_history podem não ter FK explícita
    ...
    WHERE id = ANY(v_appointment_ids);

    GET DIAGNOSTICS v_deleted_appointments = ROW_COUNT;
  END IF;

  DELETE FROM public.single_sales
  WHERE id = ANY(v_sale_ids)
    AND COALESCE(notes, '') ILIKE '%CANCELADO%';

  DELETE FROM public.package_appointments
