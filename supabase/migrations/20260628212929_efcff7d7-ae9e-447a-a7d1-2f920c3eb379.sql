...
BEGIN
  ...
  -- Não desvincula reagendados/cancelados aqui. O vínculo é histórico e só deve
  -- ser liberado pelo fluxo explícito de liberação.
  _seq := 0;
  FOR _row IN
...
BEGIN
  ...
  IF to_regprocedure('public.repair_package_cancelled_history(uuid)') IS NOT NULL THEN
  ...
  FOR rec IN
    ...
    WHERE (_client_id IS NULL OR a.client_id = _client_id)
      ...
      AND (
        (a.notes IS NOT NULL AND lower(a.notes) LIKE '%' || lower(sp.name) || '%')
        OR abs(extract(epoch from (COALESCE(pa.scheduled_date, a.start_time) - a.start_time))) <= 60 * 60 * 24 * 10
      )
    ORDER BY
      a.id,
  ...
  IF to_regprocedure('public.refresh_appointment_name_snapshots(uuid)') IS NOT NULL THEN
...
-- Repara imediatamente os pacotes já afetados.
DO $$
DECLARE
  rec record;
BEGIN
