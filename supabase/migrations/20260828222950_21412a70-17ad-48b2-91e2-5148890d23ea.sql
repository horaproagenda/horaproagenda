...
BEGIN
  ...
  -- Gate: administração/recepção ou profissional autorizado a ver outras agendas.
  ...
  SELECT * INTO p
    FROM public.professional_preferences
  ...
  SELECT a.id,
         CASE
           WHEN is_staff OR a.room_id IS NOT NULL AND EXISTS (
             SELECT 1
