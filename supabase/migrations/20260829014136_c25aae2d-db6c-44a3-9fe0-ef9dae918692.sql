...
BEGIN
  ...
  -- Permissão validada no servidor: administração/recepção ou profissional
  -- com "Ver agenda de outros profissionais" ativada.
  ...
  SELECT a.id,
         CASE
           WHEN a.room_id IS NOT NULL AND (
             is_staff OR EXISTS (
               SELECT 1 FROM public.appointments own_a
         ...
         CASE
           ...
           ) THEN r.name ELSE e.name
         END,
         a.start_time,
         a.end_time,
         a.status::text,
         -- Privacidade: bloqueio compartilhado nunca revela dados do atendimento.
   ...
   WHERE a.account_owner_id = owner_id
     ...
     -- Somente atendimentos ainda abertos ocupam o recurso.
