...
BEGIN
  ...
  RETURN QUERY
  WITH my_rooms AS (
    SELECT DISTINCT own_a.room_id AS rid
      FROM public.appointments own_a
     WHERE own_a.account_owner_id = owner_id
       AND own_a.professional_id = mine
       AND own_a.room_id IS NOT NULL
    UNION
    SELECT DISTINCT s.room_id AS rid
      FROM public.services s
     WHERE s.account_owner_id = owner_id
       AND s.professional_id = mine
       AND s.room_id IS NOT NULL
  ), my_equipment AS (
    SELECT DISTINCT own_a.equipment_id AS eid
      FROM public.appointments own_a
     WHERE own_a.account_owner_id = owner_id
       AND own_a.professional_id = mine
       AND own_a.equipment_id IS NOT NULL
  )
  SELECT a.id,
         CASE
           WHEN a.room_id IS NOT NULL
                AND (is_staff OR a.room_id IN (SELECT rid FROM my_rooms))
   ...
   WHERE a.account_owner_id = owner_id
     ...
     AND (
       is_staff
       OR (a.room_id IS NOT NULL AND a.room_id IN (SELECT rid FROM my_rooms))
       OR (a.equipment_id IS NOT NULL AND a.equipment_id IN (SELECT eid FROM my_equipment))
     );
END;
$function$;
